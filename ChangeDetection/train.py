...
import datasetsentinel as myDataLoader#changeable
...
def trainValidateSegmentation(args):
    ...
    #model = MyNet(n_classes=1, beta=512, dim = 512, numhead = 1)#default setting
    model_name="SEIFNet"
    args.savedir = args.savedir + '_' + args.file_root +model_name+ '_iter_' + str(args.max_steps) + '_lr_' + str(args.lr) + '/'
    ...
    train_data = myDataLoader.DatasetSentinel("train", "noada",file_root=args.file_root, transform=trainDataset_main)
    ...
    val_data = myDataLoader.DatasetSentinel("val", "noada", file_root=args.file_root, transform=valDataset)
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if epoch % 1 == 0 and epoch>500 and max_F1_val <= score_val['F1']:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--file_root', default="S2A", help='Data directory | LEVIR | BCDD | SYSU ')
