...
class RandCrop(object):
    """
    crop a large image to size of 256
    """
    ...
    def __call__(self, img, label):
        ...
        h0, w0, _=img.shape
        a = np.random.randint(h0-self.h)
        b = np.random.randint(w0-self.w)
        img = img[a:a+self.h,b:b+self.w,:]
        #print()
        # nearest neighbour interpolation for label image
        #label = cv2.resize(label, (self.w, self.h), interpolation=cv2.INTER_NEAREST)
        #print(label.shape)
        label = label[a:a+self.h,b:b+self.w]
        return [img, label]
...
class RandomCropResize(object):
    ...
    def __call__(self, img, label):
        if random.random() < 0.5:
            ...
            img_crop = img[y1:h - y1, x1:w - x1]#what range
            #print(img_crop.shape)
            label_crop = label[y1:h - y1, x1:w - x1]#从边上随计裁剪，是不科学的
...
class Normalize(object):
    ...
    def __call__(self, image, label):
        ...
        image = image / 255
        #label = np.ceil(label / 255)#四舍五入#这里需要进行一些变化
        label = np.ceil(label)
        for i in range(6):
            image[:, :, i] -= self.mean[i]
        for i in range(6):
...
class NormalizeSentinel(object):
    """
    Given mean: (B, G, R) and std: (B, G, R),
    will normalize each channel of the torch.*Tensor, i.e.
    channel = (channel - mean) / std
    ...
    def __call__(self, image, label):
        ...
        image = image.astype(np.float32)
        image = image / 2500
        #label = np.ceil(label / 255)#四舍五入
        label = np.ceil(label)#鉴于就是归一化的
        for i in range(6):
            image[:, :, i] -= self.mean[i]
        for i in range(6):
            image[:, :, i] /= self.std[i]
...
class ToTensor(object):
    ...
    def __call__(self, image, label):
        ...
        label_tensor = torch.LongTensor(np.array(label, dtype=np.int)).unsqueeze(dim=0)#为什么要增加维度
