...
class SupervisedAttentionModule(nn.Module):#supervised by what, by the sigmoid result?
    def __init__(self, mid_d, out_d):#, out_d
        super(SupervisedAttentionModule, self).__init__()
        self.mid_d = mid_d
        self.out_d = out_d
        # fusion
        #self.cls = nn.Conv2d(self.mid_d, 1, kernel_size=1)#mid_d to 1
        self.cls = nn.Conv2d(self.mid_d, self.out_d, kernel_size=1)#mid_d to 1
    ...
    def forward(self, x):
        mask = self.cls(x)#cls has out_d dimensions of features [b,c, h,w]
        #
        #mask=torch.cat([mask,mask,mask],dim=1)
        smask = torch.softmax(mask,dim=1)#new, we find it cant use default dim
        #print(smask.shape)
        mask_b = smask[:,:1,:,:]#the first dim bg
        #mask_b = torch.sigmoid(bmask)#is it necessary to use sigmoid, I think we cant use it
        mask_f = 1-mask_b
        #mask_f = torch.sigmoid(mask)#sigmoid is not appropriate so this methods is not for multiclass, an approprivate foreground and background assumption
        #the modification is divide the x to no change an change area
        #print(mask_f.shape)
        #print(mask_f)
        #mask_b = 1 - mask_f
        context = torch.cat([mask_f, mask_b], dim=1)#context forground and background
...
class Decoder(nn.Module):
    def __init__(self, mid_d=320, out_d=6):
        super(Decoder, self).__init__()
        self.mid_d = mid_d
        self.out_d = out_d
        # fusion
        self.sam_p5 = SupervisedAttentionModule(self.mid_d, self.out_d)#
        self.sam_p4 = SupervisedAttentionModule(self.mid_d, self.out_d)#
        self.sam_p3 = SupervisedAttentionModule(self.mid_d, self.out_d)#
        ...
        self.cls = nn.Conv2d(self.mid_d, self.out_d, kernel_size=1)#
...
class BaseNet(nn.Module):
    def __init__(self, input_nc=3, output_nc=1):
        ...
        self.decoder = Decoder(self.en_d * 2, output_nc)
